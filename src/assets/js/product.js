...
(function () {
  ...
  function wireAddToCartButton(p) {
    ...
    btn.addEventListener('click', (e) => {
      ...
      if (typeof window.addToCart === 'function' || typeof window.cart?.addToCart === 'function') {
        const addToCartFn = window.addToCart || window.cart.addToCart;
        addToCartFn({
          id: p.id || p.slug,
...
//   const IS_DEV = /^(localhost|127\.0\.0\.1)$/.test(location.hostname);
  ...
  const card = p => `
    <article class="rel-card">
      ...
      <h3 class="rel-card__title">${esc(p.title || p.name || p.slug)}</h3>
      <div class="rel-card__price">$${money(p.price ?? p.base_price ?? 0)}</div>
      <button class="rel-card__cta" type="button"
              onclick="event.preventDefault(); event.stopPropagation(); (window.addToCart || window.cart?.addToCart)({
                id: '${esc(p.id || p.slug)}',
